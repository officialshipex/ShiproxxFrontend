...
const Navbar = () => {
  ...
  useEffect(() => {
    const handler = (event) => {
      ...
      // 6) Notification dropdown — dropdown itself is nested inside each bell's
      // own wrapper now (absolute, not viewport-fixed), so isInside() on the
      // wrapper already covers clicks on the dropdown's contents too.
      if (
  ...
  // viewport-fixed pixel guess that drifted out of place at the sm: breakpoint
  // and under the admin-impersonation banner.
