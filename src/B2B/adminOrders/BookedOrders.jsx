...
const BookedOrders = ({ userId: initialUserId }) => {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            const params = {
                ...
                courier: selectedCourier.length > 0 ? selectedCourier.join(",") : undefined,
    ...
    const handleBulkGeneratePickup = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                Notification(response.data.message || "Failed to generate pickup", "error");
            }
        } catch (error) {
            Notification(error?.response?.data?.message || "Error generating pickup", "error");
        }
    };
